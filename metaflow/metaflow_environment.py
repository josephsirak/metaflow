    ...
    def _get_download_code_package_cmd(self, code_package_url, datastore_type):
        ...
        if datastore_type == "s3":
            ...
            return "{python} -c '{script}'".format(
                python=self._python(),
                script='import boto3, os; iep=os.getenv(\\"METAFLOW_S3_ENDPOINT_INTERNAL_URL\\"); ep=iep if iep else os.getenv(\\"METAFLOW_S3_ENDPOINT_URL\\"); boto3.client(\\"s3\\", **({\\"endpoint_url\\":ep} if ep else {})).download_file(\\"%s\\", \\"%s\\", \\"job.tar\\")'
                % (bucket, s3_object),
            )
